...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        ...
        setError('Email/Password registration is not enabled in your Firebase console.');
      } else {
  ...
  return (
    <div className="min-h-screen bg-hero-pattern flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Background Orbs */}
      <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-accent/20 rounded-full mix-blend-screen filter blur-[120px] animate-blob" style={{ animationDelay: '1s' }}></div>
      <div className="absolute bottom-[-10%] right-[-5%] w-[500px] h-[500px] bg-indigo-600/20 rounded-full mix-blend-screen filter blur-[100px] animate-blob" style={{ animationDelay: '4s' }}></div>

      <Card className="w-full max-w-md relative z-10 p-8 shadow-[0_0_40px_rgba(139,92,246,0.15)] border-white/20">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-purple-300 mb-3">Create Account</h1>
          <p className="text-gray-400">Join FocusRoom today to master your time</p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-gray-400">
          Already have an account? <Link to="/login" className="text-accent hover:text-white transition-colors font-semibold">Sign In</Link>
        </p>
      </Card>
    </div>
  );
